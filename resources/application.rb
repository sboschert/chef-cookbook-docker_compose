...
action :up do
  ...
  execute "running docker-compose up for project #{new_resource.project_name}" do
    ...
    only_if "[ $(docker-compose -f #{new_resource.compose_files.join(' -f ')} ps -q | wc -l) -eq 0 ]"
    user 'root'
    group 'root'
  end
end
...
action :create do
  ...
  execute "running docker-compose create for project #{new_resource.project_name}" do
...
action :down do
  ...
  execute "running docker-compose down for project #{new_resource.project_name}" do
    command "docker-compose #{get_compose_params} down  #{get_down_params}"
    environment('PATH' => '/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin')
    not_if "[ $(docker-compose -f #{new_resource.compose_files.join(' -f ')} ps -q | wc -l) -eq 0 ]"
    user 'root'
    group 'root'
  end
end
